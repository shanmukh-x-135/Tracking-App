...
begin
  ...
  insert into public.media_items (
    ...
    poster_url = coalesce(public.media_items.poster_url, excluded.poster_url),
    backdrop_url = coalesce(public.media_items.backdrop_url, excluded.backdrop_url),
    metadata = case when public.media_items.metadata = '{}'::jsonb then excluded.metadata else public.media_items.metadata end
  ...
  if coalesce(payload ->> 'recordKind', '') <> 'list_item' then
    if nullif(payload ->> 'status', '') is not null then
      ...
      if not found then
        insert into public.library_entries (user_id, media_id, status) values (current_user_id, v_media_id, payload ->> 'status')
        returning id, to_jsonb(public.library_entries), updated_at into v_target_id, applied, target_time;
        ...
        if p_conflict_policy = 'use_imported' then
          update public.library_entries set status = payload ->> 'status' where id = v_target_id
          returning jsonb_build_object('created', false, 'previous', previous, 'applied', to_jsonb(public.library_entries)), updated_at into applied, target_time;
          changed := true;
        end if;
      end if;
      if changed then
    ...
    if nullif(payload ->> 'rating', '') is not null and coalesce(payload ->> 'recordKind', '') <> 'history' then
      ...
      if not found then
        insert into public.ratings (user_id, media_id, rating) values (current_user_id, v_media_id, (payload ->> 'rating')::numeric)
        returning id, jsonb_build_object('created', true, 'applied', to_jsonb(public.ratings)), updated_at into v_target_id, applied, target_time;
        changed := true;
      elsif (previous ->> 'rating')::numeric <> (payload ->> 'rating')::numeric then
        conflicts := conflicts + 1;
        if p_conflict_policy = 'use_imported' then
          update public.ratings set rating = (payload ->> 'rating')::numeric where id = v_target_id
          returning jsonb_build_object('created', false, 'previous', previous, 'applied', to_jsonb(public.ratings)), updated_at into applied, target_time;
          changed := true;
        end if;
      end if;
      if changed then
    ...
    if nullif(payload ->> 'review', '') is not null and coalesce(payload ->> 'recordKind', '') <> 'history' then
      ...
      if not found then
        insert into public.reviews (user_id, media_id, body) values (current_user_id, v_media_id, left(payload ->> 'review', 10000))
        returning id, jsonb_build_object('created', true, 'applied', to_jsonb(public.reviews)), updated_at into v_target_id, applied, target_time;
        changed := true;
      elsif previous ->> 'body' <> payload ->> 'review' then
        conflicts := conflicts + 1;
        if p_conflict_policy = 'use_imported' then
          update public.reviews set body = left(payload ->> 'review', 10000) where id = v_target_id
          returning jsonb_build_object('created', false, 'previous', previous, 'applied', to_jsonb(public.reviews)), updated_at into applied, target_time;
          changed := true;
        end if;
      end if;
      if changed then
  ...
  if source_record.media_type = 'movie' and (payload ->> 'recordKind' = 'history' or (payload ->> 'recordKind' is null and payload ? 'watchedDate')) then
    if not exists (select 1 from public.import_provenance where user_id = current_user_id and source = source_name and source_record_key = source_record.source_record_key and target_kind = 'movie_watch_log' and undo_status = 'active') then
      insert into public.movie_watch_logs (user_id, media_id, watched_at, is_rewatch, rating, review)
      ...
      returning id, jsonb_build_object('created', true, 'applied', to_jsonb(public.movie_watch_logs)), updated_at into v_target_id, applied, target_time;
    ...
    if not exists (select 1 from public.import_provenance where user_id = current_user_id and source = source_name and source_record_key = source_record.source_record_key and target_kind = 'episode_watch_log' and undo_status = 'active') then
      insert into public.episode_watch_logs (user_id, episode_id, watched_at) values (current_user_id, v_episode_id, coalesce((payload ->> 'watchedDate')::timestamptz, now()))
      returning id, jsonb_build_object('created', true, 'applied', to_jsonb(public.episode_watch_logs)), updated_at into v_target_id, applied, target_time;
    ...
    if not exists (select 1 from public.import_provenance where user_id = current_user_id and source = source_name and source_record_key = source_record.source_record_key and target_kind = 'game_playthrough' and undo_status = 'active') then
      insert into public.game_playthroughs (user_id, media_id, status, platform, started_at, completed_at, playtime_minutes, progress_percent, rating)
      ...
      returning id, jsonb_build_object('created', true, 'applied', to_jsonb(public.game_playthroughs)), updated_at into v_target_id, applied, target_time;
    ...
    if not exists (select 1 from public.import_provenance where user_id = current_user_id and source = source_name and source_record_key = source_record.source_record_key and target_kind = 'book_reading' and undo_status = 'active') then
      insert into public.book_readings (user_id, media_id, status, started_at, finished_at, current_page, total_pages, progress_percent, rating)
      ...
      returning id, jsonb_build_object('created', true, 'applied', to_jsonb(public.book_readings)), updated_at into v_target_id, applied, target_time;
    ...
    if v_list_id is null or not exists (select 1 from public.lists item where item.id = v_list_id and item.user_id = current_user_id) then
      insert into public.lists (user_id, title, description, visibility)
      ...
      returning id, jsonb_build_object('created', true, 'applied', to_jsonb(public.lists)), updated_at into v_list_id, applied, target_time;
    ...
    if not exists (select 1 from public.import_provenance where user_id = current_user_id and source = source_name and source_record_key = source_record.source_record_key and target_kind = 'list_item' and undo_status = 'active')
      ...
      returning id, jsonb_build_object('created', true, 'applied', to_jsonb(public.list_items)), null::timestamptz into v_target_id, applied, target_time;
