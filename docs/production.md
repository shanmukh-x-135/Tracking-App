...
- Google OAuth is enabled in Supabase and sign-in has been verified on Production. TMDB, IGDB, and Google Books credentials are present only in the ignored local environment; expanded live search/detail checks and real-artwork QA pass for all three providers.
...
- Vercel project `mosaic` (`prj_xHWHkQuDW0O01jnA2SgoAWSX7bU4`) is linked to `shanmukh-x-135/Tracking-App`, with `main` as its production branch. All seven Mosaic variables are configured for Preview and Production; provider credentials use Vercel's sensitive controls. The linked Supabase integration separately supplies production-only server variables, including its default `SUPABASE_SECRET_KEY`. Mosaic does not read that key, and no temporary Management API token was uploaded to Vercel.
