...
export function HomePage() {
  ...
  const recent = useMemo(() => [...state.movieWatches.map((item) => ({ media: item.media, date: item.watchedAt, action: item.isRewatch ? "Rewatched" : "Watched", detail: item.rating ? `★ ${item.rating}` : undefined })), ...state.episodeWatches.map((item) => ({ media: item.series, date: item.watchedAt, action: `Watched S${item.seasonNumber}E${item.episodeNumber}`, detail: item.rating ? `★ ${item.rating}` : undefined })), ...state.gamePlaythroughs.map((item) => ({ media: item.media, date: item.updatedAt, action: item.status === "completed" ? "Completed" : "Updated playthrough", detail: item.progressPercent !== undefined ? `${item.progressPercent}% complete` : undefined })), ...state.bookReadings.map((item) => ({ media: item.media, date: item.updatedAt, action: item.status === "finished" ? "Finished" : "Updated reading", detail: item.totalPages ? `${item.currentPage ?? 0} / ${item.totalPages} pages` : item.progressPercent !== undefined ? `${item.progressPercent}%` : undefined }))].sort((a, b) => b.date.localeCompare(a.date)).slice(0, 6), [state]);
  ...
  return <>
    ...
    <div className="home-content">
      ...
      {user && recent.length > 0 && <section className="section"><div className="section-head"><div><span className="eyebrow">Your story</span><h2>Recently logged</h2></div><Link className="text-link" href="/activity">View activity →</Link></div><div className="recent-log-grid">{recent.map((item, index) => <Link className="recent-log-card" href={mediaHref(item.media)} key={`${mediaKey(item.media)}-${index}`}><span className="recent-log-art"><Image src={item.media.posterUrl ?? "/media-placeholder.svg"} alt="" fill sizes="72px"/></span><span><small>{item.media.mediaType === "tv" ? "Series" : item.media.mediaType}</small><strong>{item.media.title}</strong><em>{item.action}{item.detail ? ` · ${item.detail}` : ""}</em><time>{item.date.slice(0, 10)}</time></span></Link>)}</div></section>}
