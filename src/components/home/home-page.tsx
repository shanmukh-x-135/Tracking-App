...
export function HomePage({ initialDiscovery = [] }: { initialDiscovery?: CatalogMedia[] }) {
  ...
  const continueItems = useMemo(() => deriveContinue(state, { limit: 8 }), [state]);
  ...
  function scrollContinue(direction: -1 | 1): void {
    continueRailRef.current?.scrollBy({ left: direction * Math.max(280, continueRailRef.current.clientWidth * 0.9), behavior: reducedMotion ? "auto" : "smooth" });
  }
  ...
  return <>
    ...
    <div className="home-content home-atmosphere">
      {user && continueItems.length > 0 && <Reveal><section className="section continue-section" style={{ marginTop: 8 }}><div className="section-head"><div><span className="eyebrow">In progress</span><h2>Continue your stories</h2></div><div className="continue-actions"><button className="icon-button continue-scroll" type="button" aria-label="Show previous stories" onClick={() => scrollContinue(-1)}><ChevronLeft size={18}/></button><button className="icon-button continue-scroll" type="button" aria-label="Show more stories" onClick={() => scrollContinue(1)}><ChevronRight size={18}/></button><Link className="text-link" href="/library">Open library →</Link></div></div><div className="continue-rail" ref={continueRailRef} aria-label="Continue your stories">{continueItems.map((item) => <motion.article layout className="continue-card" key={`${item.kind}-${mediaKey(item.media)}`}><Link className="continue-card-link" href={mediaHref(item.media)} aria-label={`View ${item.media.title}`}/><Image src={item.media.backdropUrl ?? item.media.posterUrl ?? "/media-placeholder.svg"} alt="" fill sizes="(max-width:820px) 84vw, 31vw"/><div className="continue-body"><span className="type-badge">{item.media.mediaType === "tv" ? "Series" : item.media.mediaType}</span><h3>{item.media.title}</h3><span className="muted" style={{ fontSize: 12 }}>{item.label}</span>{item.progress !== undefined && <><div className="progress-track"><motion.div className="progress-bar" animate={{ width: `${item.progress}%` }} transition={motionTokens.slow}/></div><div className="progress-meta"><span>{item.detail}</span><span>{item.progress}%</span></div></>}{item.progress === undefined && <div className="progress-meta"><span>{item.detail}</span><span>—</span></div>}<button className="button compact" type="button" onClick={(event) => { event.stopPropagation(); window.dispatchEvent(new CustomEvent("mosaic:quick-log", { detail: item.media })); }}>{item.kind === "book" ? "Update progress" : item.kind === "game" ? "Update playthrough" : "Log episode"}</button></div></motion.article>)}</div></section></Reveal>}
