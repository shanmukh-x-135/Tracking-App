...
export function HomePage({ initialDiscovery = [] }: { initialDiscovery?: CatalogMedia[] }) {
  ...
  return <>
    ...
    <div className="home-content home-atmosphere">
      {user && continueItems.length > 0 && <Reveal><section className="section continue-section" style={{ marginTop: 8 }}><div className="section-head"><div><span className="eyebrow">In progress</span><h2>Continue your stories</h2></div><div className="continue-actions"><button className="icon-button continue-scroll" type="button" aria-label="Show previous stories" onClick={() => scrollContinue(-1)}><ChevronLeft size={18}/></button><button className="icon-button continue-scroll" type="button" aria-label="Show more stories" onClick={() => scrollContinue(1)}><ChevronRight size={18}/></button><Link className="text-link" href="/library">Open library →</Link></div></div><div className="continue-rail" ref={continueRailRef} aria-label="Continue your stories">{continueItems.map((item) => <motion.article layout className="continue-card" key={`${item.kind}-${mediaKey(item.media)}`}><Image src={item.media.backdropUrl ?? item.media.posterUrl ?? "/media-placeholder.svg"} alt="" fill sizes="(max-width:820px) 84vw, 31vw"/><div className="continue-body"><span className="type-badge">{item.media.mediaType === "tv" ? "Series" : item.media.mediaType}</span><h3>{item.media.title}</h3><span className="muted" style={{ fontSize: 12 }}>{item.label}</span><div className="progress-track"><motion.div className="progress-bar" animate={{ width: `${item.progress}%` }} transition={motionTokens.slow}/></div><div className="progress-meta"><span>{item.detail}</span><span>{item.progress}%</span></div><Link className="button compact" href={mediaHref(item.media)}>{item.kind === "book" ? "Update progress" : item.kind === "game" ? "Update playthrough" : "Log episode"}</Link></div></motion.article>)}</div></section></Reveal>}
