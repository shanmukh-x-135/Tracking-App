...
export function ProfilePage() {
  ...
  return <div className="page"><div className="page-narrow">
    ...
    {tab === "overview" && <>
      <section className="section"><div className="section-head"><div><span className="eyebrow">This year</span><h2>Your story so far</h2></div></div><div className="stats">{primaryStats.map(([value, label]) => <div className="stat" key={label}><strong>{value}</strong><span>{label}</span></div>)}</div></section>
    ...
    {tab === "stats" && <><section className="section"><div className="section-head"><div><span className="eyebrow">This year</span><h2>Tracking snapshot</h2></div></div><div className="profile-secondary-stats">{secondaryStats.map(([value, label]) => <div key={label}><strong>{value}</strong><span>{label}</span></div>)}</div></section><section className="section"><div className="profile-insights"><div className="profile-insight"><h3>Rating distribution</h3>{state.ratings.length ? <div className="insight-bars" aria-label="Rating distribution">{ratingDistribution.map(({ value, count }) => <div className="insight-bar" key={value}><span>{value}★</span><i><b style={{ width: `${(count / largestRatingBucket) * 100}%` }}/></i><strong>{count}</strong></div>)}</div> : <p className="muted">Rate a story to see your taste take shape.</p>}</div><div className="profile-insight"><h3>Logged by medium</h3>{loggedByMedium.some(({ count }) => count) ? <div className="insight-bars" aria-label="Logged by medium">{loggedByMedium.map(({ label, count }) => <div className="insight-bar" key={label}><span>{label}</span><i><b style={{ width: `${(count / largestMediumBucket) * 100}%` }}/></i><strong>{count}</strong></div>)}</div> : <p className="muted">Your real logs will appear here.</p>}</div></div></section></>}
