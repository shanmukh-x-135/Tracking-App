...
export function ProfilePage() {
  ...
  return <div className="page profile-page"><div className="page-narrow">
    ...
    <AnimatePresence mode="wait"><motion.div key={tab} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={motionTokens.normal}>{tab === "overview" && <>
      <section className="section profile-story"><div className="section-head"><div><span className="eyebrow">All time</span><h2>Your story so far</h2></div></div><div className="stats">{primaryStats.map(([value, label]) => <div className="stat" key={label}><strong>{value}</strong><span>{label}</span></div>)}</div></section>
