...
import { MediaCard, MediaShelf, mediaHref } from "@/components/media/media-card";
...
export function LibraryPage(): React.JSX.Element {
  ...
  return <div className="library-bridge-page"><div className="library-bridge-content">
    ...
    {!user && !isLoading ? <section className="empty-state"><LibraryBig size={26}/><h2>Your archive starts here</h2><p>Sign in to save movies, series, games, and books across sessions.</p><Link className="button primary" href="/login">Sign in</Link></section> : <>
      {inProgress.length > 0 && <section className="library-module in-progress-module"><div className="section-head"><div><span className="eyebrow">Keep going</span><h2>In progress</h2></div></div><div className="library-continue-grid">{inProgress.map((item) => <Link className="library-continue-card" href={mediaHref(item.media)} key={`${item.kind}-${mediaKey(item.media)}`}><Image src={item.media.backdropUrl ?? item.media.posterUrl ?? "/media-placeholder.svg"} alt="" fill sizes="(max-width:820px) 100vw, 33vw"/><span className="type-badge">{item.kind}</span><strong>{item.media.title}</strong><small>{item.label}</small>{item.progress !== undefined && <span className="library-progress"><i style={{ width: `${item.progress}%` }}/></span>}<em>{item.detail}{item.progress === undefined ? "" : ` · ${item.progress}%`}</em></Link>)}</div></section>}
