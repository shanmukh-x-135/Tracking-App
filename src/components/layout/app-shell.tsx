  ...
  return <MosaicMotion>
    <header className="desktop-nav glass">
      ...
      <div className="nav-actions">
        ...
        {!isLoading && (user ? <Link href="/profile" aria-label="Your profile"><UserAvatar name={user.displayName} avatarUrl={user.avatarUrl}/></Link> : <Link className="button" href="/login"><LogIn size={15}/>Sign in</Link>)}
      </div>
    </header>
    ...
    <nav className="mobile-nav glass" aria-label="Mobile navigation">
      <Link className={`mobile-link ${pathname === "/home" ? "active" : ""}`} href="/home"><Home size={19}/>Home</Link>
