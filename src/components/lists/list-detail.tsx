...
export function ListDetail({ listId }: { listId: string }) {
  ...
  return <div className="page"><div className="page-narrow list-detail-page">
    ...
    <div className={`list-editor-grid ${ownedList ? "" : "public"}`}><section>
      ...
      {list.items.length ? <motion.div layout className="list-editor-items"><AnimatePresence mode="popLayout">{list.items.map((item, index) => <motion.article layout className="list-editor-item" key={item.id} draggable={Boolean(ownedList)} onDragStart={() => setDraggedId(item.id)} onDragOver={(event) => { if (ownedList) event.preventDefault(); }} onDrop={() => void drop(item.id)} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={motionTokens.fast}>
      ...
      </motion.article>)}</AnimatePresence></motion.div> : <div className="history-empty">This list is empty. Search across every medium to add its first story.</div>}
