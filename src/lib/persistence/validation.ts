...
export const domainMutationSchema = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("season.state"), series: catalogMediaSchema.refine((media) => media.mediaType === "tv"), seasonNumber: z.number().int().nonnegative(), state: z.enum(["watching", "completed", "paused", "dropped"]), provenance: z.enum(["explicit_episode", "bulk_season", "imported_state"]), completedOn: z.iso.date().optional() }),
