...
export const catalogMediaSchema = z.discriminatedUnion("mediaType", [
  ...
  z.object({ ...base, mediaType: z.literal("tv"), seasonCount: z.number().int().nonnegative().optional(), episodeCount: z.number().int().nonnegative().optional(), seasonNumbers: z.array(z.number().int().nonnegative()).max(100).optional(), seasonEpisodeCounts: z.record(z.string(), z.number().int().nonnegative()).optional(), eligibleEpisodeCounts: z.record(z.string(), z.number().int().nonnegative()).optional(), eligibleEpisodeCount: z.number().int().nonnegative().optional(), seasons: z.array(z.object({ providerId: z.string().min(1).max(200), seasonNumber: z.number().int().nonnegative() })).max(1000).optional(), network: optionalText, networkLogoUrl: z.url().optional() }),
...
export const domainMutationSchema = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("episode.log"), series: catalogMediaSchema.refine((media) => media.mediaType === "tv"), seasonNumber: z.number().int().nonnegative(), episodeNumber: z.number().int().positive(), episodeTitle: z.string().trim().max(500).optional(), watchedAt: z.iso.datetime({ offset: true }), rating: rating.optional() }),
  z.object({ type: z.literal("season.state"), series: catalogMediaSchema.refine((media) => media.mediaType === "tv"), seasonNumber: z.number().int().nonnegative(), state: z.enum(["watchlist", "watching", "completed", "paused", "dropped"]), provenance: z.enum(["explicit_episode", "bulk_season", "imported_state"]), completedOn: z.iso.date().optional() }),
